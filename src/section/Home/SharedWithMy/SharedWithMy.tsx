...
export default function SharedWithMy() {
  ...
  const filteredData = sharedWithMyData.filter((item) => {
        ...
        <>
          ...
          {filteredData.length > 0 ? (
            filteredData.map((pass) => {
              return (
                <div ref={(el) => { itemRefs.current[pass._id] = el }} key={pass._id} className="data-item" >
                  <div className="item-container" onClick={() => {
                    recordUserAction(`Expand shared item: ${pass._id}`);
                    toggleExpand(pass.value, pass._id, false);
                  }}>
                    ...
                    <div className="expanded-box">
                      <p className="password-text">
                        {decrypting ? (
                          ...
                          decryptedMessages[pass._id] || "Failed to decrypt"
                        )}
                      </p>
                      ...
                      <div className="button-group">
                        <div className="action-buttons-left">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (decryptedMessages[pass._id])
                                handleCopy(decryptedMessages[pass._id]);
                            }}>
                            {copied ? "Copied!" : "Copy"}
                          </button>
                          {decryptedMessages[pass._id] && (
                            <button
                        ...
                        <div className="action-buttons-right">
                          <div className="secret-view-section">
                            <button className="view-icon-button" onClick={(e) => {
                              recordUserAction(`Button click: View stats for shared item ${pass._id}`);
                              handleGetSecretViews(e, pass._id);
                            }}>
                              <img src={showIcon} alt="view-icon" width={15} height={15} />
                            </button>
                            <span>
                              {secretViews[pass._id] ? secretViews[pass._id].totalViews : 0}
                            </span>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="shared-section">
                          ...
                          <div className="shared-users">
                            <div className="shared-user">
                              ...
                              />
                              <span><UserDisplayToggle userData={pass.sharedBy} /></span>
                            </div>
                          </div>
                        </div>
                      )}
                      {childrenLoading[pass._id] && (
